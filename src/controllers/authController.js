...
const signup = async (req, res) => {
  try {
    ...
    req.session.user = { id: result.insertId, role: 'student', name, email };
    req.session.save((err) => {
      if (err) {
        console.error('Session save error:', err);
        return res.status(500).json({ message: 'Session error during signup.' });
      }
...
const requestUserPasswordReset = async (req, res) => {
  try {
    ...
    await sendMail({
      ...
      html: `<p>Hello ${user.name},</p><p>Use the link below to reset your password. It expires in 30 minutes.</p><p><a href="${resetLink}">${resetLink}</a></p>`
    });
...
const requestAdminPasswordReset = async (req, res) => {
  try {
    ...
    await sendMail({
      ...
      html: `<p>Hello ${admin.name},</p><p>Reset your admin password within 30 minutes using the link below:</p><p><a href="${resetLink}">${resetLink}</a></p>`
    });
